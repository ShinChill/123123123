        ...
        <div>
            <div class="flex flex-row">
                ...
                <div class="basis-1/4">
                    ...
                    <a href="{{ $next_url }}" type="button" aria-label="Tập Tiếp Theo"
                        class="flex items-center bg-zinc-700 hover:opacity-80 px-2 py-1 text-xs font-medium leading-6 text-center text-gray-200 transition rounded shadow ripple hover:shadow-lg focus:outline-none waves-effect">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path strokelinecap="round" strokelinejoin="round" strokewidth="{2}"
                                d="M13 5l7 7-7 7M5 5l7 7-7 7"></path>
                        </svg>
                        <span class="pl-1 hidden md:block">Tập Tiếp</span>
                    </a>
                </div> 
                </div>
            </div>
...
<div class="py-2 px-1 bg-black border border-yellow-800 text-sm my-2">
- Hãy thử chuyển sang server khác (nếu có) nếu bạn gặp tình trạng lỗi, giật/lag khi xem phim.<br>
- Bạn nên sử dụng Trình duyệt Chrome để xem phim tối ưu nhất!<br>
- Tham gia nhóm <a href="https://t.me/+hPicJ08cKZdjNTM1" target="_blank" rel="nofollow noopener"><span style="color:#e62117"><strong>Telegram</strong></span>
</a> của chúng mình để được hỗ trợ kịp thời nha.<br>
- Do thường bị chặn truy cập vào Trang, hãy cài ứng dụng
<a href="https://1.1.1.1" target="_blank" rel="nofollow noopener"><span style="color:#e62117"><strong>1.1.1.1</strong></span></a> để vào Trang bình thường.
</div>
...
<div>
    ...
    <div class="flex flex-row flex-wrap">
        @foreach ($currentMovie->episodes->sortBy([['server', 'asc']])->groupBy('server') as $server => $data)
            <div class="mb-3">
                <span class="text-gray-300 text-sm font-bold">{{ $server }}</span>
                <div class="flex flex-row flex-wrap">
                    @foreach ($data->sortByDesc('name', SORT_NATURAL)->groupBy('name') as $name => $item)
                        <a href="{{ $item->sortByDesc('type')->first()->getUrl() }}"
                            class="text-gray-300 bg-neutral-700 hover:bg-neutral-600 font-medium text-sm min-w-[67px] text-center px-3 py-1.5 mb-1.5 mr-1.5 rounded-sm @if ($item->contains($episode)) bg-orange-900 @endif"
                            title="{{ $name }}">{{ $name }}</a>
                    @endforeach
                </div>
            </div>
...
<div class="border-t border-gray-800 border-opacity-75 pt-3 mt-2">
    ...
    <div class="mt-2 bg-[#222222] p-1 text-gray-400 text-justify">
        ...
        <div class="mt-3 pb-2 border-t border-gray-800 border-opacity-75"><i class="text-sm block pt-2 text-gray-200">Từ
                Khóa :</i> <span class="text-gray-200 text-sm">xem phim {{ $currentMovie->name }} full vietsub, thuyết minh,
                xem
                phim {{ $currentMovie->name }} full hd, {{ $currentMovie->name }} full vietsub, thuyết minh,
                motchill, motphim, phimmoi, subnhanh, vieon, netflix,
                youtube, phim1080, fullphim, dongphim, tvzing, luotphim, phimchill,
                motphimtv , netflix</span></div>
